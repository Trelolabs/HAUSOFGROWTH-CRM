...
function parseCsv(filePath: string, type: CampaignFileType, rows: ParsedRow[]): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    Papa.parse(stream as unknown as File, {
      ...
      step: (result: { data: string[] }) => {
        if (isFirst) {
          headers = result.data.map((h) => String(h).toLowerCase().trim())
          isFirst = false
          return
        }
...
function parseExcel(filePath: string, type: CampaignFileType, rows: ParsedRow[]): void {
...
export async function parseFile(
  ...
  if (ext === '.csv') {
    await parseCsv(filePath, type, rows)
  } else {
    parseExcel(filePath, type, rows)
  }

  // Always delete the temp file after parsing (engineering rule #10)
  fs.unlink(filePath, () => {})
